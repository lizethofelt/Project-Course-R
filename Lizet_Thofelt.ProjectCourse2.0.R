...
logFC <- log2(rowMeans(expression_data[, c(4, 5, 6)]) / rowMeans(expression_data[, c(1, 2, 3)]))
...
# Use all ENTREZIDs from the annotation table
universe_genes <- annotation$ENTREZID  

# Extract ENTREZ IDs from the DEG_results
entrez_ids <- DEG_results$GeneID  

# Confirm the ENTREZ IDs
print("ENTREZ IDs for KEGG enrichment:")
head(entrez_ids)

# GO Enrichment Analysis with Universe
go_enrich <- enrichGO(
  gene = entrez_ids, 
  OrgDb = org.Hs.eg.db, 
  keyType = "ENTREZID", 
  ...
  pAdjustMethod = "fdr", 
  pvalueCutoff = 0.05, 
  universe = universe_genes  # Set the background gene set
)
...
# KEGG Enrichment Analysis with Universe
